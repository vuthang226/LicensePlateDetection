...
def reconstruct(I, Iresized, Yr, lp_threshold):
    ...
    out_size = []
    lp_type = []
    out_size,lp_type = [(two_lines) if ((f.wh()[0] / f.wh()[1]) < 1.7) else (one_line) for f in final_labels_frontal],[(2) if ((f.wh()[0] / f.wh()[1]) < 1.7) else (1) for f in final_labels_frontal]
    ...
    if len(final_labels):
        ...
        for _, label in enumerate(final_labels):
            t_ptsh = getRectPts(0, 0, out_size[i][0], out_size[i][1])
            ptsh = np.concatenate((label.pts * getWH(I.shape).reshape((2, 1)), np.ones((1, 4))))
            H = find_T_matrix(ptsh, t_ptsh)
            Ilp = cv2.warpPerspective(I, H, out_size[i], borderValue=0)
            TLp.append(Ilp)
            Cor.append(ptsh)
